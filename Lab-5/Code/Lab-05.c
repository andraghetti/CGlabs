...
static GLuint textureName[TEXTURES+1];
static GLuint checkerTexture;
static GLuint cubeMapTexture;
static GLuint bumpMap;
...
enum TexMode currentTexMode = TEXTUREMAP2D;
...
void putVert(int i, int j) {
	...
	float verticalRate = (j%NumPerWrap) / (float)NumPerWrap;
	float horizontalRate = (i%NumWraps) / (float)NumWraps;

	float phi = PI2*verticalRate;
	float theta = PI2*horizontalRate;
	...
	if (currentTexMode == TEXTUREMAP2D) { 
		...
		selectedTexture == 1 ? //fix momentaneo: non riesco a girare le bmp da sistema operativo
...
void display( void ) {
	...
	case MODE_TORUS:
		...
		switch (currentTexMode)
		{
			case TEXTUREMAP2D:
				glEnable(GL_TEXTURE_2D);
				glBindTexture(GL_TEXTURE_2D, textureName[selectedTexture]);
				break;
			case ENVMAPSPHERE:
				glEnable(GL_TEXTURE_2D);
...
void initTextures( const char* filenames[] ) { 
	int i;
	glGenTextures(TEXTURES+1, textureName );	// Load four texture names into array
...
void initProceduralTextures() { 
	...
	glBindTexture(GL_TEXTURE_2D, textureName[TEXTURES]); 

	for (i = 0; i<128; i++) {
		for (j = 0; j<128; j++) {
			c = ((((i & 0xF)) | (((j & 0xF)))))*255;
			//c = (i+j) % 255;
			image[i][j][0] = (GLubyte)c%(i+1);
			image[i][j][1] = (GLubyte)c%(j+1);
			image[i][j][2] = (GLubyte)c%(i/2+j/2+1);
		}
	}
...
void initCubeMap() {
	...
	RgbImage posx = getTextureFromFile(cubeMap[0]);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, internalFormat, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, ImageData(&posx));
	
	RgbImage negx = getTextureFromFile(cubeMap[1]);
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, internalFormat, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, ImageData(&negx));
	
	RgbImage posy = getTextureFromFile(cubeMap[2]);
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, internalFormat, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, ImageData(&posy)); 
	
	RgbImage negy = getTextureFromFile(cubeMap[3]);
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, internalFormat, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, ImageData(&negy));
	
	RgbImage posz = getTextureFromFile(cubeMap[4]);
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, internalFormat, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, ImageData(&posz));
	
	RgbImage negz = getTextureFromFile(cubeMap[5]);
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, internalFormat, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, ImageData(&negz));
...
void subMenuTexture(int sel) {
	if (sel!=-1 && (currentTexMode == TEXTUREMAP2D || currentTexMode == ENVMAPSPHERE))
		selectedTexture = sel;
	glutPostRedisplay();
}
...
void menu(int sel) {
	...
	case TEXTUREMAP2D:
	case ENVMAPCUBE:
		selectedTexture = 0; //first
		break;
	case ENVMAPSPHERE:
		selectedTexture = TEXTURES-2; //always reflect 1
		break;
	}
...
char* subString(const char* input, int prefix,int suffix, char* dest) {
	int input_len = strlen(input);

	strncpy(dest, input + prefix, input_len-prefix-suffix);
	dest[input_len - prefix - suffix] = '\0';
	return dest;
}
// Main routine
...
int main( int argc, char** argv ) {
	...
	int i, idTexture;
	char dest[80];
	idTexture = glutCreateMenu(subMenuTexture);
	for (i = 0; i < TEXTURES; i++) {
		glutAddMenuEntry(subString(textures[i], 12, 4, dest), i);
	}
	glutAddMenuEntry("Procedural texture", TEXTURES); //procedurale per ultima
	glutCreateMenu(menu);
	glutAddSubMenu("TEXTURES", idTexture);

	glutAddMenuEntry("Texture Map", TEXTUREMAP2D);
	glutAddMenuEntry("Environment Sphere", ENVMAPSPHERE);
	glutAddMenuEntry("Environment Cube", ENVMAPCUBE);
	...
	return(10);	// This line is never reached.
}
