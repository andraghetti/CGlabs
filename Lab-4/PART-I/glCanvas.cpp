...
GLCanvas::TraceRay (float i, float j)
{
  ...
  int samples = args->num_antialias_samples;
  ...
  if (samples > 1) 
  {
	  for (int i = 0; i < samples; i++) {
		  ...
		  float dx = (float)(rand()- RAND_MAX) / (max_d*RAND_MAX);
		  float dy = (float)(rand()- RAND_MAX) / (max_d*RAND_MAX);

		  Ray r = camera->generateRay(Vec2f(x + dx, y + dy));
